...
buildSequence('seq-root-cu04',
  ...
  [
    { from:0, to:1, label:'asignarRuta()', pat:'GRASP Controller', chip:'Controller',
      txt:'El OperadorLogístico delega al Controller. Punto de entrada único: toda la lógica se orquesta desde aquí.' },
    { from:1, to:2, label:'snapshot()', pat:'Memento',
      txt:'Antes de modificar nada, Memento guarda un snapshot inmutable. Si algo falla, el estado se puede restaurar.' },
    { from:1, to:3, label:'elegirVehículo()', pat:'Strategy',
      txt:'Strategy 1: AsignadorDeVehiculos. Criterio intercambiable en runtime — PorCapacidad o PorDisponibilidad — sin tocar el Controller.' },
    { from:1, to:4, label:'elegirRuta()', pat:'Strategy',
      txt:'Strategy 2: CalculadorDeRuta. Criterio intercambiable: MasCercana, MasBarata o MenosCongestionada.' },
    { from:1, to:5, label:'cambiarEstado()', pat:'State',
      txt:'Fuerza la transición a EN_TRANSITO. cambiarEstado() no valida — la validación vive en los métodos de comportamiento (entregar(), retener()…).' },
    { from:5, to:6, label:'notificarObservadores()', pat:'Observer',
...
buildSequence('seq-root-cu07',
  [
    ...
    { nm: 'Decorator',     role: 'DecoradorRastreoGPS' },
    { nm: 'Envío',         role: 'Envio' },
    { nm: 'Estado',        role: 'EstadoEnTransito' },
    { nm: 'Dashboard',     role: 'DashboardObservador' },
    { nm: 'Email/SMS',     role: 'SistemaNotificacionObservador' },
    { nm: 'Centro',        role: 'CentroDistribucionObservador' },
  ],
  [
    { from:0, to:1, label:'reportarPosicion()', pat:'Decorator',
      txt:'DecoradorRastreoGPS apila el tracking sobre Envío sin modificar la clase base — la interfaz es idéntica.' },
    { from:1, to:2, label:'entregar()', pat:'State',
      txt:'El Decorator delega en Envío. entregar() delega en el estado actual: EstadoEnTransito valida que la transición a EN_REPARTO sea legal.' },
    { from:2, to:3, label:'update(evento)', pat:'Observer',
      txt:'cambiarEstado() llama a notificarObservadores(). El Dashboard refresca el mapa y el ETA automáticamente.' },
    { from:2, to:4, label:'update(evento)', pat:'Observer',
      txt:'El mismo evento llega a SistemaNotificacionObservador. El cliente recibe el SMS — Envío no sabe quién escucha.' },
    { from:2, to:5, label:'update(evento)', pat:'Observer',
      txt:'CentroDistribucionObservador actualiza su ocupación. Tres suscriptores distintos, un solo notificar — abierto a extensión, cerrado a modificación.' },
  ],
  'Seguí el seguimiento en vivo: Decorator → State → Observer fan-out. Pulsá "Siguiente paso" para empezar.'
);
