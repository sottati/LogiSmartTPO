...
buildSequence('seq-root-cu04',
  ...
  [
    ...
    { from:1, to:6, label:'cambiarEstado(new EstadoEnTransito())', pat:'State',
      txt:'Fuerza la transición a EN_TRANSITO — cambiarEstado() no valida, solo asigna el nuevo estado. La validación ocurre en los métodos de comportamiento (entregar(), retener()…).' },
    { from:6, to:7, label:'notificarObservadores()', pat:'Observer',
...
buildSequence('seq-root-cu07',
  ...
  [
    ...
    { from:2, to:3, label:'entregar()', pat:'State',
      txt:'entregar() delega en el estado actual: EstadoEnTransito.entregar() valida que la transición a EN_REPARTO sea legal — la validación vive en el estado, no en el Envío.' },
    { from:3, to:2, ret:true, label:'ok', pat:'State',
      txt:'EstadoEnTransito llama a cambiarEstado(new EstadoEnReparto()). Cada estado decide sus propias transiciones — sin switch gigante en el Envío.' },
